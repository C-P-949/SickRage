...
class Logger(object):  # pylint: disable=too-many-instance-attributes
    ...
    def log(self, msg, level=INFO, *args, **kwargs):
        ...
        check2 = re.sub(r'error \[SSL: SSLV3_ALERT_HANDSHAKE_FAILURE\] sslv3 alert handshake failure \(_ssl.c:\d{3}\)', 'See: http://git.io/vuU5V', message)
        if check != message or check2 != message:
            message = check
            level = WARNING
